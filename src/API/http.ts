// src/api/http.ts
// 使用 Web Crypto API 进行 JWT 签名和验证
...
interface Env {
  ...
  AUTH_PASSWORD_HASH?: string; // 认证密码的哈希值（SHA-256）
  AUTH_SECRET?: string; // JWT密钥（必须至少32字节）
}
...
export class NavigationAPI {
  ...
  constructor(env: Env) {
    ...
    if (this.secret.length < 32) {
      throw new Error('AUTH_SECRET 必须至少 32 字节，请使用强随机密钥');
    }
  }
  ...
  async login(loginRequest: LoginRequest): Promise<LoginResponse> {
    ...
    // 验证用户名和密码（使用密码哈希）
    const isPasswordValid = await this.verifyPassword(loginRequest.password, this.passwordHash);

    if (loginRequest.username === this.username && isPasswordValid) {
      // 生成JWT令牌，传递记住我参数
  ...
  async verifyToken(token: string): Promise<{ valid: boolean; payload?: Record<string, unknown> }> {
    ...
    try {
      ...
      const computedSignature = await this.hmacSha256(dataToSign, this.secret);
      ...
      return { valid: true, payload: decodedPayload as Record<string, unknown> };
    } catch (error) {
      // Token 无效、过期或签名错误
      return { valid: false };
    }
  ...
  private async generateToken(
    ...
    // 使用 HMAC-SHA256 签名
    const dataToSign = `${encodedHeader}.${encodedPayload}`;
    const signature = await this.hmacSha256(dataToSign, this.secret);
