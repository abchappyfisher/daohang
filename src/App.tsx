...
import { sanitizeCSS, validateCSSUrl } from './utils/sanitizer';
...
function App() {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      logger.debug('认证检查结果:', result);

      if (!result) {
        // 未认证，需要登录
        logger.debug('未认证，设置需要登录状态');

        // 如果有token但无效，清除它
        if (api.isLoggedIn()) {
          logger.debug('清除无效token');
          api.logout();
        }
  ...
  useEffect(() => {
    ...
    const backgroundImage = configs['site.backgroundImage'];
    ...
    // 验证背景图片 URL
    if (backgroundImage && !validateCSSUrl(backgroundImage)) {
      logger.error('无效的背景图片URL，已移除');
    }
  ...
  const handleSaveSiteOrder = async (groupId: number, sites: Site[]) => {
    try {
      logger.debug('保存站点排序', groupId, sites);
