    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      try {
        ...
        if (api.isAuthEnabled()) {
          ...
          // CSRF 保护 - 验证 CSRF Token（仅对修改请求）
          if (['POST', 'PUT', 'DELETE', 'PATCH'].includes(method)) {
            const csrfToken = request.headers.get('X-CSRF-Token');

            if (!csrfToken) {
              return new Response('缺少CSRF保护令牌', { status: 403 });
            }

            // 验证 CSRF Token（使用用户的 session 信息）
            // 在实际应用中，应该将 CSRF Token 与用户 session 关联
            // 这里简化处理，只要存在 CSRF Token 即可
            // 注意：这是一个基础实现，生产环境需要更严格的验证
          }
        }
        ...
        // 站点相关API
          ...
          if (data.url !== undefined) {
            ...
            if (!urlValidation.valid) {
              return Response.json(
                {
                  success: false,
                  message: urlValidation.error || '无效的URL格式',
                },
                { status: 400 }
              );
            }
          ...
          if (data.icon !== undefined && data.icon !== '') {
            const iconValidation = validateUrl(data.icon);
            if (!iconValidation.valid) {
              return Response.json(
                {
                  success: false,
                  message: iconValidation.error || '无效的图标URL格式',
                },
                { status: 400 }
              );
            }
...
function validateSite(data: SiteInput): {
  ...
  if (!data.url || typeof data.url !== 'string') {
    ...
    const urlValidation = validateUrl(data.url);
    if (!urlValidation.valid) {
      errors.push(urlValidation.error || '无效的URL格式');
    } else {
  ...
  if (data.icon !== undefined) {
    if (typeof data.icon !== 'string') {
      ...
      const iconValidation = validateUrl(data.icon);
      if (!iconValidation.valid) {
        errors.push(iconValidation.error || '无效的图标URL格式');
      } else {
